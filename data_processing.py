...
my_filtered_table = my_table3.filter(lambda x: 'ia' in x['team'] and float(
    x['minutes']) < 200.0 and float(x['passes']) > 100.0)
...
# =======
# Pivot table test
my_pivot = my_table5.pivot_table(['embarked', 'gender', 'class'], ['fare', 'fare', 'fare', 'last'], [
...
my_pivot = my_joined_table.pivot_table(['EU', 'coastline'], ['temperature', 'latitude', 'latitude'], [
                                       lambda x: sum(x)/len(x), lambda x: min(x), lambda x: max(x)])
print(my_pivot)

my_pivot = my_table5.pivot_table(['class', 'gender', 'survived'], [
                                 'survived', 'fare'], [lambda x: len(x), lambda x: sum(x)/len(x)])
print(my_pivot)
# >>>>>>> pivot_feature
